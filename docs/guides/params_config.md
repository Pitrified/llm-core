...
|---|---|---|
| **Config** | `src/llm_core/config/` | Defines the *shape* of settings as Pydantic models |
| **Params** | `src/llm_core/params/` | Loads *actual values* and produces a Config via `to_config()` |
...
- [`SampleConfig`](../../reference/llm_core/config/sample_config/) - config side
- [`SampleParams`](../../reference/llm_core/params/sample_params/) - params side
...
1. **Single constructor argument**: `env_type: EnvType | None = None`.
   Pass it explicitly from the caller (e.g., `LlmCoreParams`).  When
...
from llm_core.params.env_type import (
    EnvLocationType, EnvStageType, EnvType,
    UnknownEnvLocationError, UnknownEnvStageError,
)
from llm_core.config.sample_config import NestedModel, SampleConfig
...
`LlmCoreParams` (the top-level singleton) owns all Params instances
...
```python
from llm_core.params.env_type import EnvLocationType, EnvStageType, EnvType
