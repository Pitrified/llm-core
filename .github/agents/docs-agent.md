...
- **Key Patterns:** Modular architecture in `src/`; configuration in `src/llm_core/config/`; testing in `tests/`.
- **Relevant Folders:**
  - `src/llm_core/` – Main application code
  - `src/llm_core/config/` – Configuration management
...
- Ensure `.env` file is not present in the root, must be in `~/cred/llm_core/.env`.
- Secrets should not be committed.

## Related

- `src/llm_core/config/llm_core_paths.py`
