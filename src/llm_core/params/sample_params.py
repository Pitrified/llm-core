...
``src/llm_core/config/sample_config.py``.

Config / Params split:

* **Config** (``src/llm_core/config/``) defines only the *shape* of
...
See Also:
    ``SampleConfig`` - the paired config model in ``src/llm_core/config/``.
...
from llm_core.config.sample_config import NestedModel
from llm_core.config.sample_config import SampleConfig
